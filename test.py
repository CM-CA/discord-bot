...
class Temporizador(Thread):
    ...
    def run(self):
        ...
        hora = hora.replace(
            hour=aux.hour, minute=aux.minute, second=aux.second, microsecond=0
        )
        ...
        print(
            "Proxima ejecución programada el {0} a las {1}".format(
                hora.date(), hora.time()
            )
        )
        ...
        while self._estado:
            ...
            if hora <= datetime.now():
                self.funcion()
                print(
                    "Ejecución programada ejecutada el {0} a las {1}".format(
                        hora.date(), hora.time()
                    )
                )
                hora += timedelta(days=1)
                print(
                    "Próxima ejecución programada el {0} a las {1}".format(
                        hora.date(), hora.time()
                    )
                )
...
t = Temporizador("21:07:00", 1, ejecutar)  # Instanciamos nuestra clase Temporizador
...
sleep(
    120
)  # Simulamos un tiempo de espera durante el cual el programa principal puede seguir funcionando.
